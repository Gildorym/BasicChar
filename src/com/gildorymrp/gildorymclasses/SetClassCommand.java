		...
		if ((args.length >= 2) && (sender.hasPermission("basicchar.command.setclass"))) {
			if (CharacterClass.valueOf(args[1].toUpperCase()) != null) {
				if (Bukkit.getServer().getPlayer(args[0]) != null) {
					...
					this.plugin.classes.put(Bukkit.getServer().getPlayer(args[0]).getName(), CharacterClass.valueOf(args[1].toUpperCase()));
				} else {
					sender.sendMessage(ChatColor.RED + "That player is not online!");
				}
			}
			else sender.sendMessage(ChatColor.RED + "That's not a valid class!");
		}
		else if (args.length >= 1)
			try {
				sender.sendMessage(ChatColor.GREEN + "Class set to " + CharacterClass.valueOf(args[0].toUpperCase()).toString());
				this.plugin.classes.put(sender.getName(), CharacterClass.valueOf(args[0].toUpperCase()));
